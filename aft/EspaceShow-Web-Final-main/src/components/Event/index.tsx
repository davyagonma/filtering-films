...
export default function CreerEvent() {
    ...
    const [code, setEventCode] = useState(generateRandomCode())
    const [nom, setName] = useState('')
    const [category, setCategory] = useState('')
    const [pays, setPays] = useState('Benin')
    const [ville, setVille] = useState('')
    const [description, setDescription] = useState('')
    const [heure, setHeure] = useState('')
    const [date_debut, setHeureDebut] = useState('')
    const [date_fin, setHeureFin] = useState('')
    const [adresse, setAdresse] = useState('')
    const [mots_cles, setMotsCles] = useState("")
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setPlace(sessionStorage.getItem('totalPlace') ? sessionStorage.getItem('totalPlace')! : "");
        }, 1000);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (res.success) {
            toast({
                title: "Evenement créé avec succès",
                description: "Vous recevrez une notification après sa validation",
                variant: "default"
            })
            clearAllTickets()
            ...
            <section className="p-5 pt-[100px] min-h-screen w-[100%] md:w-[auto] ">
                ...
                <div className="flex justify-start">
                    ...
                    <div className="w-full md:w-[auto] md:ml-[150px] flex flex-col justify-center md:justify-start  ">
                        <p style={{ color: `${colors.primary}` }} className="text-xl text-center md:text-start font-bold">Créer un événement</p>
                        <div className="">
                                ...
                                <div className="flex justify-center items-center" >
                                    <form id="event-form" className="pt-10 pb-5 w-full col-auto md:flex md:justify-start md:items-start md:gap-20" onSubmit={(e) => handleSubmit(e as React.FormEvent<HTMLFormElement>)}>
                                            ...
                                            <div className="flex justify-between items-center pb-5 w-full">
                                                ...
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        className="p-2 rounded-lg text-black text-center w-[100px] ml-4 blur-sm"
                                                        style={{
                                        ...
                                        <div className="w-full md:w-[350px] md:pt-[50px]">
                                            ...
                                            <div>
                                                <button type="submit" className="rounded-lg p-[13px] bg-[#C30F66] text-white w-full mt-10 text-sm ">Créer l'événement</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
